...
id __cdecl ALTDeviceIDReplacementFunction()
{
	...
	auto deviceIDString = MiniappBuilderCore::instance()->serverID();
...
{
	fs::path appleFolderPath(MiniappBuilderCore::instance()->appleFolderPath());
	if (!fs::exists(appleFolderPath))
	{
		throw AnisetteError(AnisetteErrorCode::iTunesNotInstalled);
	}

	fs::path internetServicesDirectoryPath(MiniappBuilderCore::instance()->internetServicesFolderPath());
	...
	fs::path applicationSupportDirectoryPath(MiniappBuilderCore::instance()->applicationSupportFolderPath());
...
bool AnisetteDataManager::ReprovisionDevice(std::function<void(void)> provisionCallback)
{
	...
	// Copy existing MiniappBuilder .pb files into original location to reuse the MID.
	for (const auto& entry : fs::directory_iterator(adiDirectoryPath))
	{
		if (entry.path().extension() == ".miniappBuilder")
		{
	...
	auto cleanUp = [adiDirectoryPath]() {
		...
		// Backup MiniappBuilder ADI files.
		for (const auto& entry : fs::directory_iterator(adiDirectoryPath))
		{
			// Backup miniappBuilder file
			if (entry.path().extension() == ".pb")
			{
				fs::path backupPath = entry.path();
				backupPath += ".miniappBuilder";
	...
	try
	{
		...
		// Call callback while machine is provisioned for MiniappBuilder.
		provisionCallback();
	}
...
bool AnisetteDataManager::ResetProvisioning()
{
	...
	// Remove existing miniappBuilder .pb files so we can create new ones next time we provision this device.
	for (const auto& entry : fs::directory_iterator(adiDirectoryPath))
	{
		if (entry.path().extension() == ".miniappBuilder")
		{
